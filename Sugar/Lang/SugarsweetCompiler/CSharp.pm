...
package Sugar::Lang::SugarsweetCompiler::JavaScript;
use parent 'Sugar::Lang::SugarsweetBaseCompiler';
	...
	sub code_function_preamble {
		...
		my $return_type = $self->compile_variable_type($function_tree->{return_type});
		if ((0 < scalar(@{$function_tree->{argument_list}}))) {
			my $argument_list = $self->compile_argument_list($function_tree->{argument_list});
	...
	sub is_my_native_function {
		my ($self, $function_tree) = @_;
		if (($function_tree->{native_type} eq 'csharp')) {
			return 1;
		} else {
			return 0;
		}
	...
	sub compile_statement {
		...
		if (($statement->{type} eq "foreach_statement")) {
			...
			my $expression = $self->compile_condition_expression($statement->{expression});
			...
			if (exists($statement->{branch})) {
				...
				while ($branch) {
					if (($branch->{type} eq 'elsif_statement')) {
						my $expression = $self->compile_condition_expression($branch->{expression});
			...
			if (($self->{current_function_tree}->{return_type} eq 'void')) {
				push @{$code}, "return;";
			} else {
	...
	sub compile_condition_expression {
		my ($self, $expression) = @_;
		if (($expression->{type} eq 'variable_expression')) {
	...
	sub compile_expression {
		...
		if (($expression->{type} eq 'string_expression')) {
			...
			my $match_code = "Regex.Match($sub_expression, \@\"$compiled_regex->{regex}\")";
	...
	sub compile_regex_expression {
		...
		if (($regex_token =~ /\A\/((?:[^\\\/]|\\.)*+)\/([msixpodualngc]*+)\Z/s)) {
			...
			return { regex => ($regex), flags => ($flags), has_g => ($has_g), token => ($token) };
		} else {
	...
	sub compile_argument_list {
		...
		foreach my $arg (@{$argument_list}) {
			if (($i > 0)) {
				my $arg_type = $self->compile_variable_type($arg->{variable_type});
	...
	sub compile_variable_type {
		my ($self, $type_token) = @_;
		if (($type_token eq "tree")) {
			return "Dictionary<string, object>";
		} elsif (($type_token eq "list")) {
			return "List<object>";
		} elsif (($type_token eq "string")) {
			return 'string';
		} elsif (($type_token eq "int")) {
			return 'int';
		} else {
			return 'void';
		}
