...
sub context_root {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '=') {
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'package' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'package_identifier') {
			$self->{save_tokens_index} = $self->{tokens_index};
			$context_value->{package_identifier} = $tokens[1][1];
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'package' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier') {
			$self->{save_tokens_index} = $self->{tokens_index};
			$context_value->{package_identifier} = $tokens[1][1];
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'tokens' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{') {
			$self->{save_tokens_index} = $self->{tokens_index};
			$context_value->{tokens} = $self->context_token_definition([]);
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'ignored_tokens' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{') {
			$self->{save_tokens_index} = $self->{tokens_index};
			$context_value->{ignored_tokens} = $self->context_ignored_tokens_list([]);
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'item' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'context' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier') {
			$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'list' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'context' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier') {
			$self->{save_tokens_index} = $self->{tokens_index};
			my $var_context = { type => 'list_context', line_number => $tokens[0][2], identifier => $tokens[2][1], block => $self->context_action_block, };
			push @{$context_value->{contexts}}, $var_context;
			$context_value->{contexts_by_name}{$tokens[2][1]} = $var_context;
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'object' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'context' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier') {
			$self->{save_tokens_index} = $self->{tokens_index};
			my $var_context = { type => 'object_context', line_number => $tokens[0][2], identifier => $tokens[2][1], block => $self->context_action_block, };
			push @{$context_value->{contexts}}, $var_context;
			$context_value->{contexts_by_name}{$tokens[2][1]} = $var_context;
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'sub' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'code_block') {
			$self->{save_tokens_index} = $self->{tokens_index};
...
sub context_def_value {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'string') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'string_value', line_number => $tokens[0][2], value => $tokens[0][1], };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'substitution_regex') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'substitution_regex_value', line_number => $tokens[0][2], value => $tokens[0][1], };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'regex') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'regex_value', line_number => $tokens[0][2], value => $tokens[0][1], };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'variable') {
			$self->{save_tokens_index} = $self->{tokens_index};
...
sub context_token_definition {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '}') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return $context_value;
...
sub context_match_item {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'function_reference' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '->') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'function_match', line_number => $tokens[0][2], function => $tokens[0][1], argument => $self->context_spawn_expression, };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'function_reference') {
			$self->{save_tokens_index} = $self->{tokens_index};
...
sub context_match_action {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'variable' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '=') {
			$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'variable' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{') {
			$self->{save_tokens_index} = $self->{tokens_index};
			...
			$self->{save_tokens_index} = $self->{tokens_index};
			if ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[3] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{') {
				$self->{save_tokens_index} = $self->{tokens_index};
				my $var_subkey_expression = $self->context_spawn_expression;
				$self->confess_at_current_offset('"}", "=" expected after sub-key expression')
					unless (($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[4] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '}' and ($tokens[5] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '=';
				$self->{save_tokens_index} = $self->{tokens_index};
			...
			} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[3] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '[') {
				$self->{save_tokens_index} = $self->{tokens_index};
			...
			$self->{tokens_index} = $self->{save_tokens_index};
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'match') {
			$self->{save_tokens_index} = $self->{tokens_index};
			my $var_statement = { type => 'match_statement', line_number => $tokens[0][2], match_list => $self->context_match_list_specifier, };
			if ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'or' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'die') {
				$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'switch') {
			$self->{save_tokens_index} = $self->{tokens_index};
			$self->confess_at_current_offset('expected \'{\'')
				unless (($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{';
			$self->{save_tokens_index} = $self->{tokens_index};
			push @$context_value, { type => 'switch_statement', line_number => $tokens[0][2], switch_cases => $self->context_switch_blocks([]), };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'while') {
			$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'warn') {
			$self->{save_tokens_index} = $self->{tokens_index};
...
sub context_switch_blocks {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '}') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return $context_value;
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'default') {
			$self->{save_tokens_index} = $self->{tokens_index};
...
sub context_if_chain {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'elsif') {
			$self->{save_tokens_index} = $self->{tokens_index};
			...
			return $context_value;
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'else') {
			$self->{save_tokens_index} = $self->{tokens_index};
...
sub context_spawn_expression {
	...
	while ($self->{tokens_index} < @{$self->{tokens}}) {
		...
		if ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] =~ /\A(\$\d++)\Z/) {
			$self->{save_tokens_index} = $self->{tokens_index};
			if ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'line_number' and ($tokens[3] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '}') {
				$self->{save_tokens_index} = $self->{tokens_index};
				return { type => 'get_token_line_number', line_number => $tokens[0][2], token => $tokens[0][1], };
			} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'line_offset' and ($tokens[3] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '}') {
				$self->{save_tokens_index} = $self->{tokens_index};
			...
			$self->{tokens_index} = $self->{save_tokens_index};
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '$_') {
			$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 3 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'identifier' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq ':' and ($tokens[2] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '{') {
			$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[1] eq 'undef') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'undef', line_number => $tokens[0][2], };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'context_reference' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '->') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'call_context', line_number => $tokens[0][2], context => $tokens[0][1], argument => $self->context_spawn_expression, };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 1 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'context_reference') {
			$self->{save_tokens_index} = $self->{tokens_index};
			return { type => 'call_context', line_number => $tokens[0][2], context => $tokens[0][1], };
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'function_reference' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '->') {
			$self->{save_tokens_index} = $self->{tokens_index};
		...
		} elsif ((($self->{tokens_index} = $self->{save_tokens_index}) + 2 <= @{$self->{tokens}}) and ($tokens[0] = $self->{tokens}[$self->{tokens_index}++])->[0] eq 'substitution_regex' and ($tokens[1] = $self->{tokens}[$self->{tokens_index}++])->[1] eq '->') {
			$self->{save_tokens_index} = $self->{tokens_index};
